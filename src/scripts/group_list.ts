...
async function renderGroups() {
  ...
  try {
    ...
    myGroupsContainer.innerHTML = `<div class="alert alert-danger">Erro ao carregar grupos: ${(err as Error).message}</div>`;
  }
}

function renderGroupCard(g: Group): string {
  const date = g.last_post
    ? new Date(g.last_post).toLocaleString("pt-BR", { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })
    : null;
      ...
      <a href="/src/pages/group_detail.html?slug=${g.slug}"
        ...
        <div class="card-body d-flex flex-column">
          ...
          <p class="card-subtitle mb-2 small text-gray">
            criado por ${g.created_by__username} <span class="meta-dot"></span>
